async function getWeather() {
  ...
  try {
    ...
    document.getElementById(
      "temperature"
    ).textContent = `Temperature: ${currentData.main.temp}°C`;
    ...
    ).innerHTML = `<img src="https://openweathermap.org/img/wn/${currentIcon}@2x.png" alt="weather icon">`;
    ...
    forecastDays.forEach((day, index) => {
      ...
      const weekday = new Date(forecast.dt_txt).toLocaleDateString("en-US", {
        weekday: "long",
      });
      day.querySelector(".weekday").textContent = weekday;
      day.querySelector(
        ".icon"
      ).innerHTML = `<img src="https://openweathermap.org/img/wn/${forecastIcon}@2x.png" alt="forecast icon">`;
      day.querySelector(".temp").textContent = `${Math.round(
        forecast.main.temp
      )}°C`;
    });
  } catch (error) {
